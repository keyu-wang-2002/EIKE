...
class Train(nn.Module):
    def __init__(self,args,dataset):
        ...
        self.concept_vec.weight.requires_grad = True
        ...
        nn.init.normal_(self.concept_vec.weight.data, 0.0, 1.0 / args.emb_dim)
        ...
        for epoch in range(nepoch):
            ...
            if epoch >= 1000:
                if epoch % 200 < 100:
                    ...
                    self.concept_vec.weight.requires_grad = False
            ...
            if epoch == 0:
                ...
                #write for cpp test
                with open("./vector/transEllipsoid/"+self.args.dataset +"/entity2vec" + str(epoch + 1) + ".vec", 'w') as file:
                    for vec in entity_vec_save:
                        list_vec = list(vec)
                        str_vec = "\t".join([str(x) for x in list_vec])
                        file.write(str_vec+"\n")

                with open("./vector/transEllipsoid/"+ self.args.dataset + "/relation2vec" + str(epoch + 1) + ".vec", 'w') as file:
                    for vec in relation_vec_save:
                        list_vec = list(vec)
                        str_vec = "\t".join([str(x) for x in list_vec])
                        file.write(str_vec+"\n")

                with open("./vector/transEllipsoid/" + self.args.dataset+"/concept2vec" + str(epoch + 1) + ".vec", 'w') as file:
                    for vec, r, r_axis in zip(concept_vec_save, concept_r_save, concept_axis_save):
                        list_vec = list(vec)
            ...
            for batch in range(nbatches): #nbatches
                ...
                self.concept_vec.weight.data = normalize_emb(self.concept_vec.weight.data)
    ...
    def doTrainInstanceOf(self, ids):
        ...
        dis = F.relu(norm(torch.div(entity_embs - concept_embs, radius_axis), pnorm=2) - radius ** 2)
    ...
    def doTrainSubClassOf(self, ids):
        ...
        dis = F.relu(norm(torch.div(concept_embs_a, radius_a_axis) - torch.div(concept_embs_b, radius_b_axis), pnorm=2)
...
def parseargs():
    ...
    parsers.add_argument("--nepoch", type=float, default=1000) #default=1000
