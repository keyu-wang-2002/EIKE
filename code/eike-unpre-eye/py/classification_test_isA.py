...
class TestIsA:
    def __init__(self, data_set, epoch, ex_rate):
    ...
    def load_vector(self):
        ...
        f2 = open("vector/" + self.data_set + "/concept_in2vec" + str(self.epoch) + ".vec", 'r', encoding='utf-8')
        f3 = open("vector/" + self.data_set + "/concept_ex2vec" + str(self.epoch) + ".vec", 'r', encoding='utf-8')
        ...
        while True:
            ...
            self.concept_in_vec.append(line_concept_list)
        ...
        while True:
            ...
            line = line.strip('\n').split('\t')
            line_list = list(map(float, line))
            self.concept_ex_vec.append(line_list)
    ...
    def test(self):
        ...
        def check_instance(instance, concept):
            dis_in = np.sum(np.square(np.true_divide(self.instance_vec[instance] - self.concept_in_vec[concept],
                                           self.concept_r[concept][:-1]))) - np.square(self.concept_r[concept][-1])
            dis_in = dis_in if dis_in > 0 else 0
            dis_ex =  1 - cosine_similarity(self.instance_vec[instance], self.concept_ex_vec[concept])
            score = dis_in  + dis_ex * self.ex_rate
            if self.get_max_min:
        ...
        def check_sub_class(concept1, concept2):
            dis_in = np.sum(np.square(np.true_divide(self.concept_in_vec[concept1], self.concept_r[concept1][:-1]) -
                            np.true_divide(self.concept_in_vec[concept2], self.concept_r[concept2][:-1])))  + \
                             np.square(self.concept_r[concept1][-1]) - np.square(self.concept_r[concept2][-1])
            dis_in = dis_in if dis_in > 0 else 0
            dis_ex = 1 - cosine_similarity(self.concept_ex_vec[concept1], self.concept_ex_vec[concept2]) +  \
                             np.linalg.norm(self.concept_ex_vec[concept1], axis=-1) - np.linalg.norm(self.concept_ex_vec[concept1], axis=-1)
            dis = dis_in + dis_ex * self.ex_rate
            score = dis
            if np.sqrt(dis_in) < np.fabs(self.concept_r[concept1][-1] - self.concept_r[concept2][-1]) and \
                    self.concept_r[concept1][-1] < self.concept_r[concept2][-1]:
                return True
            if np.sqrt(dis_in) < self.concept_r[concept1][-1] + self.concept_r[concept2][-1]:
                tmp = (self.concept_r[concept1][-1] + self.concept_r[concept2][-1] - np.sqrt(dis_in) - dis_ex * self.ex_rate) / self.concept_r[concept1][-1]
