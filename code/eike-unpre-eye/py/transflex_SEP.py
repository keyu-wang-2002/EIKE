...
class Dataset(object):
    def __init__(self, args):
        ...
        self.relation_vec,self.entity_vec,self.concept_in_vec = [],[],[]
...
class Train(nn.Module):
    def __init__(self,args,dataset):
        ...
        self.concept_in_vec.weight.requires_grad = True
        ...
        nn.init.normal_(self.concept_in_vec.weight.data, 0.0, 1.0 / args.emb_dim)
    ...
    def doTrain(self):
        ...
        for epoch in range(nepoch):
            ...
            if epoch == 0:
                ...
                concept_in_vec_save = self.concept_in_vec.weight.detach().cpu().numpy()
                concept_ex_vec_save = self.concept_in_vec.weight.detach().cpu().numpy()
                ...
                #             "concept_in_vec": concept_in_vec_save,
                ...
                with open("./vector/" + self.args.dataset+"/concept_in2vec" + str(epoch + 1) + ".vec", 'w') as file:
                    for vec, r, r_axis in zip(concept_in_vec_save, concept_r_save, concept_axis_save):
                        list_vec = list(vec)
                ...
                with open("./vector/" + self.args.dataset+"/concept_ex2vec" + str(epoch + 1) + ".vec", 'w') as file:
                    for vec in concept_ex_vec_save:
                        list_vec = list(vec)
                        str_vec = "\t".join([str(x) for x in list_vec])
                        file.write(str_vec+"\n")
            ...
            for batch in range(nbatches):
                ...
                self.concept_in_vec.weight.data = normalize_emb(self.concept_in_vec.weight.data)
    ...
    def doTrainInstanceOf(self, ids):
        ...
        concept_in_embs = self.concept_in_vec(ids[[1, 3], :])
        ...
        dis_in = F.relu(norm(torch.div(entity_embs - concept_in_embs, radius_axis), pnorm=2) - radius ** 2)
        dis_ex = 1 - torch.cosine_similarity(concept_ex_embs, entity_embs, -1)
        dis = dis_in + dis_ex * self.args.ex_rate
        loss = F.relu(dis[0] + self.args.margin_ins - dis[1]).sum()
        return loss

    def doTrainSubClassOf(self, ids):
        concept_in_embs_a = self.concept_in_vec(ids[[0, 2], :])
        concept_in_embs_b = self.concept_in_vec(ids[[1, 3], :])
        ...
        dis_in = F.relu(norm(torch.div(concept_in_embs_a, radius_a_axis) - torch.div(concept_in_embs_b, radius_b_axis), pnorm=2)
                     + radius_a ** 2 - radius_b ** 2)
        dis_ex = 1 - torch.cosine_similarity(concept_ex_embs_a, concept_ex_embs_b, -1) + torch.norm(concept_ex_embs_a, dim=-1) - torch.norm(concept_ex_embs_b, dim=-1)
        dis = dis_in + dis_ex * self.args.ex_rate
...
def parseargs():
    ...
    parsers.add_argument("--ex_rate", type=float, default=0.3)
