...
class Dataset(object):
    def __init__(self, args):
        ...
        self.relation_vec,self.entity_vec,self.concept_ex_vec = [],[],[]
...
class Train(nn.Module):
    def __init__(self,args,dataset):
        ...
        self.concept_ex_vec = nn.Embedding(self.D.concept_num, args.emb_dim)
        self.concept_r = nn.Embedding(self.D.concept_num, 1)
        # self.concept_in_vec = nn.Embedding(self.D.concept_num, args.emb_dim)
        self.concept_in_vec = nn.Embedding.from_pretrained(self.pretrained_vector())
        ...
        self.entity_vec.weight.requires_grad = True
        self.concept_ex_vec.weight.requires_grad = True
        self.concept_in_vec.weight.requires_grad = False
        ...
        nn.init.normal_(self.concept_ex_vec.weight.data, 0.0, 1.0 / args.emb_dim)
        nn.init.normal_(self.concept_in_vec_linear.weight.data, 0.0, 1.0 / args.emb_dim)
    ...
    def doTrain(self):
        ...
        for epoch in range(nepoch):
            ...
            if epoch >= 1000:
                if epoch % 200 < 100:
                    self.entity_vec.weight.requires_grad = False
                    self.concept_ex_vec.weight.requires_grad = False
                    self.concept_in_vec.weight.requires_grad = False
                    self.concept_in_vec_linear.weight.requires_grad = True
            ...
            if epoch == 0:
                ...
                concept_ex_vec_save = self.concept_ex_vec.weight.detach().cpu().numpy()
                #concept_in_vec_save = self.concept_ex_vec.weight.detach().cpu().numpy()
                concept_in_vec_save = self.concept_in_vec_linear(self.concept_in_vec.weight).detach().cpu().numpy()
                ...
                #             "concept_ex_vec": concept_ex_vec_save,
                #             "relation_vec":relation_vec_save},file)
                #print("saved!")

                #write for cpp test
                with open("./vector/"+self.args.dataset +"/entity2vec" + str(epoch + 1) + ".vec", 'w') as file:
                ...
                with open("./vector/" + self.args.dataset+"/concept_ex2vec" + str(epoch + 1) + ".vec", 'w') as file:
                    for vec, r, r_axis in zip(concept_ex_vec_save, concept_r_save, concept_axis_save):
                        list_vec = list(vec)
                        ...
                        str_r = str(r[0])
                        file.write(str_vec + "\n" + str_r_axis + "\t" + str_r + "\n")
                with open("./vector/" + self.args.dataset+"/concept_in2vec" + str(epoch + 1) + ".vec", 'w') as file:
                    for vec in concept_in_vec_save:
                        list_vec = list(vec)
                        str_vec = "\t".join([str(x) for x in list_vec])
                        file.write(str_vec+"\n")
            ...
            for batch in range(nbatches):
                ...
                self.concept_in_vec_linear.weight.data = normalize_emb(self.concept_in_vec_linear.weight.data)
    ...
    def doTrainInstanceOf(self, ids):
        ...
        concept_ex_embs = self.concept_ex_vec(ids[[1, 3], :])
        radius = torch.squeeze(self.concept_r(ids[[1, 3], :]), dim=-1)
        radius_axis = self.concept_axis(ids[[1, 3], :])

        concept_in_embs = self.concept_in_vec_linear(self.concept_in_vec(ids[[1, 3], :]))

        dis_ex = F.relu(norm(torch.div(entity_embs - concept_ex_embs, radius_axis), pnorm=2) - radius ** 2)
        dis_in = 1 - torch.cosine_similarity(concept_in_embs, entity_embs, -1)
        dis = dis_ex + dis_in * self.args.in_rate
        loss = F.relu(dis[0] + self.args.margin_ins - dis[1]).sum()
        return loss

    def doTrainSubClassOf(self, ids):
        concept_ex_embs_a = self.concept_ex_vec(ids[[0, 2], :])
        concept_ex_embs_b = self.concept_ex_vec(ids[[1, 3], :])
        ...
        concept_in_embs_a = self.concept_in_vec_linear(self.concept_in_vec(ids[[0,2],:]))
        concept_in_embs_b = self.concept_in_vec_linear(self.concept_in_vec(ids[[1,3],:]))

        dis_ex = F.relu(norm(torch.div(concept_ex_embs_a, radius_a_axis) - torch.div(concept_ex_embs_b, radius_b_axis), pnorm=2)
                     + radius_a ** 2 - radius_b ** 2)
        dis_in = 1 - torch.cosine_similarity(concept_in_embs_a, concept_in_embs_b, -1) + (torch.norm(concept_in_embs_a, dim=-1) - torch.norm(concept_in_embs_b, dim=-1))
        dis = dis_ex + dis_in * self.args.in_rate
...
def parseargs():
    ...
    parsers.add_argument("--nepoch", type=float, default=1000) # 1000
...
def main():
    ...
    train = Train(args = args,dataset= dataset)
    train.doTrain()
