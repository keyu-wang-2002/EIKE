...
cosine_similarity = lambda vec1, vec2: vec1.dot(vec2) / (np.linalg.norm(vec1) * np.linalg.norm(vec2))
...
class TestIsA:
    ...
    def load_vector(self):
        f1 = open("vector/" + self.data_set + "/entity2vec" + str(self.epoch) + ".vec", 'r', encoding='utf-8')
        f2 = open("vector/" + self.data_set + "/concept_ex2vec" + str(self.epoch) + ".vec", 'r', encoding='utf-8')
        f3 = open("vector/" + self.data_set + "/concept_in2vec" + str(self.epoch) + ".vec", 'r', encoding='utf-8')
        ...
        while True:
            ...
            self.concept_ex_vec.append(line_concept_list)
            self.concept_r.append(line_r_list)

        self.concept_in_vec = list()
        while True:
            line = f3.readline()
            if not line:
                break
            line = line.strip('\n').split('\t')
            line_list = list(map(float, line))
            self.concept_in_vec.append(line_list)

        self.concept_ex_vec = np.array(self.concept_ex_vec)
        self.concept_r = np.array(self.concept_r)
        self.instance_vec = np.array(self.instance_vec)
        self.concept_in_vec = np.array(self.concept_in_vec)
        return True
    ...
    def test(self):
        ...
        def check_instance(instance, concept):
            dis = np.sum(np.square(np.true_divide(self.instance_vec[instance] - self.concept_ex_vec[concept],
                                           self.concept_r[concept][:-1])))
            dis_in =  1 - cosine_similarity(self.instance_vec[instance], self.concept_in_vec[concept])
            score = dis - np.square(self.concept_r[concept][-1]) + dis_in * self.in_rate

            # record max and min delta
            if self.get_max_min:
        ...
        def check_sub_class(concept1, concept2):
            dis_ex = np.sum(np.square(np.true_divide(self.concept_ex_vec[concept1], self.concept_r[concept1][:-1]) -
                            np.true_divide(self.concept_ex_vec[concept2], self.concept_r[concept2][:-1])))
            dis_in = 1 - cosine_similarity(self.concept_in_vec[concept1], self.concept_in_vec[concept2]) +  \
                       np.linalg.norm(self.concept_in_vec[concept1], axis=-1) - np.linalg.norm(self.concept_in_vec[concept2], axis=-1)
            if np.sqrt(dis_ex) < np.fabs(self.concept_r[concept1][-1] - self.concept_r[concept2][-1]) and \
                    self.concept_r[concept1][-1] < self.concept_r[concept2][-1]:
                return True
            if np.sqrt(dis_ex) < self.concept_r[concept1][-1] + self.concept_r[concept2][-1]:
                tmp = (self.concept_r[concept1][-1] + self.concept_r[concept2][-1] - np.sqrt(dis_ex) - dis_in * self.in_rate) / self.concept_r[concept1][-1]
...
def test_isA(data_set, epoch, in_rate):
    test_isa_example = TestIsA(data_set, epoch, in_rate)
    result_tuple = test_isa_example.run()
    return result_tuple


if __name__ == '__main__':
    test_isA('YAGO39K', 1000, 0.01)